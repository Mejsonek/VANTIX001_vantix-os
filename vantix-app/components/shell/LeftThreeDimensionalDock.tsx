...
export default function LeftThreeDimensionalDock() {
  ...
  return (
    <div className="flex flex-col h-screen w-[60px] bg-void border-r border-gold/[0.06] relative z-20">
      {/* Logo */}
      <div className="flex items-center justify-center h-12 border-b border-gold/[0.06]">
      ...
      {/* Shell Modules */}
      <div className="flex-1 flex flex-col items-center py-3 gap-[2px]">
        {shellItems.map((item) => {
          ...
          return (
            <Link
              key={item.id}
              href={item.href}
              className="group relative flex items-center justify-center w-full py-[6px]"
            >
              {active && (
              ...
              `}>
                <Icon
                  size={14}
                  className={`transition-colors duration-150 ${active ? 'text-gold' : 'text-ivory/25 group-hover:text-ivory/50'}`}
                />
                {item.badge && (
                  <span className="absolute -top-[3px] -right-[3px] w-3.5 h-3.5 flex items-center justify-center bg-gold text-void text-[6px] font-bold font-mono">
                    {item.badge}
                  </span>
                )}
              </div>
              <div className="absolute left-full ml-2 px-2 py-1 bg-void border border-gold/15 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 whitespace-nowrap pointer-events-none z-50">
                <span className="font-mono text-[9px] text-ivory">{item.label}</span>
                {item.shortcut && (
                  <span className="ml-1.5 font-mono text-[6px] text-ivory/25 border border-ivory/10 px-1 py-[1px]">{item.shortcut}</span>
                )}
              </div>
            </Link>
          );
      ...
      {/* System Modules */}
      <div className="flex flex-col items-center py-2 gap-[2px]">
          ...
          return (
            ...
            >
              ...
              `}>
                <Icon
                  size={12}
                  className={`transition-colors duration-150 ${active ? 'text-gold/70' : 'text-ivory/15 group-hover:text-ivory/35'}`}
                />
              </div>
              <div className="absolute left-full ml-2 px-2 py-1 bg-void border border-gold/15 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 whitespace-nowrap pointer-events-none z-50">
                <ChevronRight size={8} className="inline text-gold/30 mr-1" />
                <span className="font-mono text-[8px] text-ivory/60">{item.label}</span>
              </div>
            </Link>
          );
        })}
